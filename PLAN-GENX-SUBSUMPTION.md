...
**Status:** approved; spec-first execution under way. honest-format spec drafted (`specs/03-application-production/honest-format-architecture.md`) and registered in the build order (DOM → format → components); `fmtx`/`smartx` are its reference of record. **honest-format is COMPLETE** at `javascript/honest-format/` — coercion + hf-type conversion, the full format vocabulary (numeric, text, temporal, structured + compact), smart detection, the declared vocabulary manifest (`manifest.json`), and the DOM boundary (`bind.js`), all passing the five JS gates. **HC-REF004 in honest-check is also done**, closing the static-reference family. honest-format is the first fully-landed genX-subsumed Tier 3 module. Remaining genX spokes (uix/dragx/loadx/accx → honest-components, navx → honest-page, bindx → honest-DOM) unstarted — each needs its Tier 3 spec written/extended first. Deferred within honest-format: JSON hf-type conversions (object/array/json), and email/url smart→link rendering (currently verbatim). **Source library:** `~/dev/genX` (MIT, ~20k LoC JS + TS bindings + CSS), a declarative HTML-attribute client runtime. **Target:** the application-production tier (`specs/03-application-production/`) and its `javascript/` reference implementation.
