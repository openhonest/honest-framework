...
**Status:** approved; spec-first execution under way. honest-format spec drafted (`specs/03-application-production/honest-format-architecture.md`) and registered in the build order (DOM → format → components); `fmtx`/`smartx` are its reference of record. **Implementation under way** at `javascript/honest-format/` — increments 1 (value coercion + hf-type conversion) and 2 (`format.js`: the numeric and text formatter families + bestDenominator) pass all five JS gates. Next honest-format increments: the temporal formats (date/time/datetime/relative — needs an injected `now`) and `compact`, then `detect.js` (smart), `manifest.js` (unblocks HC-REF004), `bind.js`, and the phone/ssn/creditcard family. Remaining spokes (uix/dragx/loadx/accx → components, navx → page, bindx → DOM) unstarted.
