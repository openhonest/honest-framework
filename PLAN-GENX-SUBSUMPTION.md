...
**Status:** approved; spec-first execution under way. honest-format spec drafted (`specs/03-application-production/honest-format-architecture.md`) and registered in the build order (DOM → format → components); `fmtx`/`smartx` are its reference of record. **Implementation under way** at `javascript/honest-format/` — the pure formatter core is COMPLETE across four increments (value coercion + hf-type conversion; numeric + text families + bestDenominator; temporal; structured phone/ssn/creditcard + compact), all passing the five JS gates. The pure detection core (`detect.js`) and the declared vocabulary (`manifest.js` + emitted `manifest.json`) are done. Next: (a) build the **HC-REF004 consumer in honest-check** — resolve authored `hf-*` attribute values against `manifest.json`, closing the static-reference family at last; (b) `bind.js` (the DOM boundary: scan/formatElement, the §8.3 processed-marker predicate, the `smart` and email/url delegation, the injected `now` for relative, and the deferred JSON hf-type conversions). Remaining spokes (uix/dragx/loadx/accx → components, navx → page, bindx → DOM) unstarted.
