...
| **classify** | Two distinct jobs that unfortunately share the word. (1) *honest-type's classify* — recognizing an untrusted, open input into one of its declared kinds, at the server request boundary; this is the type system doing real recognition. (2) *the client's "classify"* — the bootloader parsing a declared `h*-` attribute's tokens (including the unordered "Type Magic" form) and dispatching each to its module, by matching against the closed vocabularies honest-type declares (used as data). The second is parse-and-dispatch of already-declared values, not recognition of untrusted input, and needs no honest-type engine on the client. When a spec says the bootloader "classifies," it means sense (2). |
...
| **mutation adequacy** | A measure of a test suite, beyond coverage: change the source in small mechanical ways, one at a time from a fixed list (a comparison loosened, a number shifted by one, a line deleted), and require some test to fail on each change. A change no test catches is a line no test would notice was wrong — a gap coverage cannot show, because the unchanged line still ran. Measured only against a suite that already passes on the unchanged source. |
| **caught (change)** | A change to the source that makes at least one test fail — proof that some test would notice that mistake in the real code. (The mutation-testing literature calls this a *killed* mutant.) |
| **set-aside change** | A change that cannot alter any result, so no test could fail on it — listed by name with the reason it is harmless, kept distinct from a change nothing caught. Allowed only when the harmlessness is demonstrated, not merely asserted. (The literature calls this an *equivalent* mutant.) |
| **uncaught change** | A change that no test fails on and that was not set aside: a place where a mistake could slip through unnoticed. The gate stops on any. (The literature calls this a *surviving* mutant.) |
