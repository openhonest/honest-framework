...
- **User state is declared in the templates, never discovered at runtime** — the templates say which parts of the page are user state, and generation is deterministic, so it is statically knowable and checkable. If you think a user-state rule "needs the running app," you have missed this.
...
**What is open and what is commercial.** The `.hd` format and the tooling that *reads* it are FOSS; the tooling that *authors it visually* is commercial. Concretely: the format specification (its grammar and semantics), the reference reader (parser → intermediate representation → validator), and the static renderer (intermediate representation → the 4-column diagram) are open, under the framework's standard licence tiers. The interactive design surface — where an architect draws the diagram and the tool emits `.hd`, enforcing the boundary invariant at composition time — is the commercial product, protected by its own trademark, patent on the composition-interaction model, and certification programme, not by the format. This split is deliberate: a file format is not meaningfully protectable, and an open format with an open reader is what lets every other tool and AI agent *consume* `.hd`, which is exactly what makes the commercial visual *producer* valuable. Reading and viewing `.hd` is open; authoring it visually is the commercial line.
...
**Every change to the code must make at least one test fail.** The source is changed in small, mechanical ways from a fixed list — a comparison loosened from `<` to `<=`, a number shifted by one, a condition flipped, a line deleted — and each changed version is run against the whole suite. If some change makes no test fail, then no test would catch that same mistake in the real code. Coverage cannot reveal this, because the unchanged line still ran. The list of changes is finite and applied in full, the same way the input vocabularies are. This is measured only against a suite that already passes on the unchanged source: a suite failing for any other reason would register every change as caught and prove nothing. A change that cannot alter the result is set aside — but only when that is *demonstrated* (the change leaves the suite's observed output identical), never merely asserted, since an unjustified set-aside re-creates one level up the very self-checking gap this requirement closes. Such changes come in a few recurring kinds — a guard on a value the parser always supplies, a trailing return that falls through to the same value, a constant shifted in a way that preserves order — each listed by name with its reason, not left to pass silently.

The first requirement means a test cannot check itself against itself. The second means that if a line could be wrong without any test failing, the gate stops. The second also *enforces* the first: a test that takes its expected answer from the code under test cannot fail when that code is changed, so it surfaces as a change nothing caught — self-checking is not a separate thing to police by inspection, it shows up structurally as an uncaught change. Coverage shows the line ran; these two show that a mistake in the line would be caught.
...
**User state is in the templates, and that is not a runtime question — READ THIS BEFORE YOU CONCLUDE ANYTHING IS "ONLY KNOWN AT RUNTIME."** The same logic covers what the page *holds*, not only what it *sends*. Which parts of the page are user state — and how to read each one — is declared in the templates: the DATAOS manifest and the elements its selectors point at. Code generation from those templates is deterministic — the same templates always produce the same code — so nothing about user state is discovered while the app runs. It is fixed by the templates and fully knowable before a line executes. Therefore any rule about user state — for example, forbidding a second copy of it kept anywhere but the page — is checkable statically, by reading the templates, at the gate. The reflex "a tool can't know which data is user state, that's a runtime thing" is wrong every time: **it is in the templates, and generation is deterministic.** This trips up readers (and tools) repeatedly; it should never trip you up again.
...
```
# Leaf — depends on nothing. The one true first module, hand-checked.
parse                                              the shared parser (wraps tree-sitter)

# The .hd architecture-declaration layer (the FOSS read path; authoring tooling, not an adopter
# runtime). Second only to parse: every module below it, type included, is declared in .hd.
design     → parse                                 the .hd language layer: the tree-sitter .hd
                                                   ...
                                                   diagram). Comes right after parse and before type —
...
# Leaf dependency-wise (depends on nothing), but declared in .hd, so it follows design.
...
- **`design` second, right after `parse` and before `type`.** The `.hd` layer needs only the parser (its own tree-sitter `.hd` grammar, alongside the jinja/css grammars), so it can come as soon as `parse` exists; `parse` is the one true leaf, the single artifact that must precede everything, `design` included. It comes *before* `type` — and before the whole code-quality tier — because `.hd` is the layer in which every subsequent module, `type` included, *declares* its architecture, and everything downstream is checked against those declarations. Its one hard consumer is honest-check's **conformance tier** (the rules that verify an implementation matches its declared architecture — role reachability, orchestrator discipline), which reads `design`'s IR. honest-check's **universal tier** needs only `parse` and fires without any `.hd`, so it is independent of `design`; only the conformance pass depends on `design`'s IR and lands once `design` exists. The `.hd` format and the framework's own `.hd` declarations are hand-authored and hand-checked first, before the reader automates the check — the same posture as `parse` being "the shared parser, checked by hand first." `design` is authoring/dev tooling shipped in the repo, never a runtime dependency of an adopter's app. Its full architecture is in `../02-code-quality/honest-design-architecture.md`.
...
4. **Gate everything else.** Every other module — and re-checking the first three — lands only by passing all four gates: honest-check (shape), its own conformance suite (behaviour), full line-and-branch coverage, and mutation adequacy (every change to its source caught or set aside). No code that fails the framework's own gate enters the repository.
