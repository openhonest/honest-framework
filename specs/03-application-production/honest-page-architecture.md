...
- Dynamic theme authoring and visual token management — out of scope for this standard. FOSS theming is honest-page's `--ht-` base tokens plus `light-dark()` (§7), combined with the components' own `var()` fallback defaults and the runtime's merged token contract (honest-components §6.4)
