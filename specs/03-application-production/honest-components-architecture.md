...
2. **Custom-property tokens.** Values components read via `var(--...)` but never set, so the host can override them: honest-page's `--ht-` base tokens (honest-page §7) plus each organism's own tokens, defaulted by their `var()` fallbacks and declared in `style.json` as the override contract the runtime merges at startup (§6.4). Tokens are the *only* visual coupling between components.
...
CSS custom properties (`--color-primary`, `--button-height`, etc.) are global by definition. The namespace scan never touches custom property declarations or references. The organism's `var(--data-table-bg, #fff)` reads a global token — defaulted by its fallback, overridable by the host (§6.4) — so the scan does not prefix it.

### 6.4 The Token File and Token Resolution

A component carries its own default styling, and every token it owns it reads with an inline fallback — `var(--data-table-bg, #fff)`. The fallback is the default. A component **reads** its tokens but never **sets** them, which is exactly what keeps them overridable: because the value lives nowhere in the component but the fallback, an application that declares `--data-table-bg` earlier in the cascade wins, and the fallback applies only where the application says nothing. So a token is defaulted and overridable at once, by the cascade, with no runtime value resolution. Dark mode is honest-page's `light-dark()` mechanism (honest-page §7): a fallback may itself be `light-dark(#fff, #000)`, and the browser resolves it.

**Normative (FOSS) path.** At startup, the component runtime reads the style manifest (`style.json`) from every installed component and collects every declared token across all components into one **token contract** — the merged, human-readable list of every customization point the installed set exposes. The runtime injects no default values: the defaults are the components' own `var()` fallbacks, so the contract is documentation and a duplicate check, not a value store. Two components must never declare the same token key: every token is block-namespaced (`--data-table-bg`, `--button-height`), so a duplicate is a namespace violation, not two values to reconcile. The merge **fails loudly at startup** on a duplicate key — it never silently picks a winner — so a clash is caught immediately, not left to produce styling that depends on component load order. This token contract, the components' fallback defaults, and honest-page's `--ht-` base tokens are the complete FOSS theming story. A conformant application themes fully with nothing more.
...
`style.json` lists keys and descriptions, not values: it is the override contract, not where a token's default lives. The default is the component's own `var(--...)` fallback in its CSS (§6.4); `style.json` names each token so the host knows what it may override and the gate can check the bijection. The component declares only what it needs.
