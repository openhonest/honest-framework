...
1. **Pure functions** — `@recognizer`, `@helper`, and any `@link` that does no I/O. Tested by **running the inputs the declarations allow**, with the strategy taken from what each declaration actually bounds. A Set is finite, so every member and every combination runs: `format_name(5) × currency_code(150) × style_name(4)` is 3,000 cases, all of them, no sampling. A bounded length runs every valid length, plus the two that must be rejected: one under the minimum and one over the maximum. A numeric range bounds the value but leaves the set of values infinite, so it is probed at limits rather than enumerated, taking Fibonacci values outward from zero in both directions to a configured ceiling, dense among small numbers and spaced among large ones. A predicate that bounds nothing enumerable — an external lookup, or a lower bound with no maximum — runs the values the programmer supplied, and a missing entry is reported, not passed over. Each result says which of these it did: probing is never reported as exhaustion. The honesty checks ride along here: each pure function is run twice on the same input and must give the same answer, must not change its input, and a chain of pure functions run twice must match; recognizers also get the near-miss inputs (one character changed, look-alike letters, control characters) and must reject them. (Sections 2, 3, 4.)
...
3. **I/O boundary functions** — `@link(boundary=True)` / `@boundary`: the actual reads and writes (database, DOM, network). They touch the outside world, so they cannot be run purely, but nothing about them is simulated. Their input is closed like every other input: the manifest that reaches them was derived at intake from the templates. Their output is checked against the **real mechanism, never a mock**. The one seam is the injected connection, and what gets injected is a genuine driver: a real in-memory database, recreated fresh for each test, with the declared schema applied and the genuine query builders and `execute` run against it. The assertion is what the real store actually did, so there is nothing standing in that could drift from how the real thing behaves. This works for any store because what is injected is the store instance, not a substitute for its behaviour. (Sections 5, 6.)

Sitting above all three is **BDD**: developer-authored Gherkin `.feature` files. Every roled function carries **exactly one gherkin scenario** stating its behaviour, so the mapping is one function, one gherkin, and a roled function without one is an HC-P009 diagnostic. That one-to-one mapping is what lets the gherkin count serve as a direct function-point count. Chain-level gherkins are filed one file per chain and carry the multi-step journeys no single function's test can express; function-level gherkins are filed alongside their function. Scaffolding is auto-generated from `@link` declarations; the developer writes only the `.feature` files. (Section 8.)
...
- Visual appearance, and only the part of it that nothing declares. Appearance in this framework is declarative: a component namespaces under its own BEM block, a page sets `--ht-` custom-property tokens, and a behaviour attribute takes its value from a closed vocabulary. Each of those is a reference that must resolve to a definition, so honest-check settles it statically against the classes and tokens the project defines (honest-framework-spec, *Every reference resolves, or the gate stops*). What is left once they resolve is non-functional: it does not change what the program does, so it is not a correctness question and no test should report on it. Everything else about a rendered surface is covered, and is covered because the server generates it: the page is HTML the server rendered, so the markup, the tokens it can send back, and the identifiers it points at are fixed by the templates and readable before anything runs (*The input boundary is closed*). Structure is therefore checked statically by honest-check, not here; what honest-test adds is the behavioural half — asserting the response a chain actually emits carries the declared content type and the declared selectors (§8.4).
    ...
    FOR EACH test_function IN test_module:
        FOR EACH statement IN test_function.body:
            IF statement uses a monkeypatch fixture (setattr/setitem/delattr/setenv),
               ...
               OR rebinds an imported symbol by attribute assignment (imported.attr = replacement):
...
A gherkin is not reserved for chains. **Every roled function — `@link`, `@recognizer`, `@boundary`, `@helper` — carries exactly one gherkin scenario** stating its behavior. A pure internal helper is specified by a gherkin just as a top-level chain is. The governing model is honest-gherkin-architecture.md §9.
...
A change whose run does not complete is **undecided**. Each run is bounded in time and in memory, and a run that hits either bound produced no verdict: nothing observed the change, so nothing may be claimed about it. Recording such a run as caught asserts a detection that did not happen, and it fails in the direction that hides the defect — on a loaded machine more runs hit the bound, so more real survivors are reported as caught and the measure reports an adequacy it never established. The bound is a property of the harness and the machine it runs on, not of the change: elapsed time cannot distinguish a change that stopped the program from a machine that was busy.

Adequacy therefore requires two things, not one: caught plus set aside equals the total, **and** no change is left undecided. An undecided change is disclosed by name with the bound it hit, the way a set-aside change is, and it fails the gate exactly as an unexamined change does, because a change nothing judged is not evidence that the suite would catch it. A change that genuinely does not halt is then declared in the set-aside record with that as its reason, which puts the judgment on the record and with a person rather than inferring it from a clock.

The check that the suite passes on the unchanged source runs through the same path that judges every change. A check that reaches the suite by another route cannot see a fault in the route that does the judging, and this is the one measurement the rest of the gate rests on.
