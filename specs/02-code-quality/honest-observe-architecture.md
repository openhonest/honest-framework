...
5. **Projections** — pure functions that fold the event log into derived read models: dashboards, metrics, timelines, aggregate counts, audit trails.
...
```
emit(event_type, aggregate_type, aggregate_id, payload, context) → Result
```
...
| `context` | dict | The current manifest. honest-observe extracts `auth` and `meta` from it automatically using the configured field names. |

**Return:**

`ok({ event_id: "..." })` on success. `err({ code: "emit_failed", ... })` if the log write fails. The caller decides whether to propagate the fault.
...
FUNCTION emit(event_type, aggregate_type, aggregate_id, payload, context):
    event ← {
        ...
        event_version:  event_type_registry[event_type].version,
    ...
    IF "err" IN result:
        RETURN err({
            code:     "emit_failed",
            category: "server",
            message:  "Failed to append event to log",
            detail:   result["err"]
        })
...
Sequence numbers are per-aggregate, monotonically increasing. They provide ordering guarantees within an aggregate without requiring global locks. Two events with the same `aggregate_id` will have different sequence numbers. Two events with different `aggregate_ids` may have identical sequence numbers.

Sequence numbers are generated by honest-persist using a per-aggregate counter. The implementation is language-idiomatic; the contract is that sequence numbers are unique and increasing within an aggregate.
...
For high-cardinality aggregates (millions of events), replaying the full log on every request is too slow. honest-observe supports snapshot projections: the projection state is periodically persisted to honest-persist, and subsequent replays start from the snapshot rather than the beginning.
...
hlc = {
    ...
    logical:  Integer,   # monotonic counter for same-millisecond events
