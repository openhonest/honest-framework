...
| honest-persist | **SPEC-COMPLETE** | FK cascade, view/trigger/proc DDL, full inspector (SQLite + PostgreSQL), and materialized views all built | §6.6 materialized-view emulation (backing table + refresh triggers + registry) and §9.1 full-schema inspector done for SQLite/Turso and PostgreSQL (tables + views + triggers + procedures via `_hp_object`). PostgreSQL is verified at the module's standard bar — pinned catalog SQL + reconstruction from representative `information_schema` rows, not a live server (no PG in the harness). RETURNING/upsert remain by-design out-of-scope |
