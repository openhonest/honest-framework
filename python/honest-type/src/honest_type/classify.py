...
from honest_type.vocabulary import auto_binding
...
def _resolve_bindings(tickets, rejections, bind):
    """Pass 2 (section 6), unit-2 subset: flat binding, all slots required."""
    manifest = {}
    ...
    for type_name, slot in bind.items():
...
def classify(tokens, vocab, bind=None):
    ...
    return _resolve_bindings(tickets, rejections, binding)
