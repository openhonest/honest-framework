...
| **Elementary process** | The unit honest-estimate counts: the smallest activity meaningful to a user OR enforceable as a system-boundary contract. Read from the `.hd` (a boundary/orchestrator role, or a pure function no sibling invokes); an invoked helper is covered by its caller, not counted. The spec-tier grain, distinct from the per-`def` code-honesty census. |
| **Breadth / depth** | honest-estimate's two deductive size numbers, never fused. Breadth = the count of elementary processes (IFPUG-comparable). Depth = bits of specified case-distinction, the sum of log-2 of each closed vocabulary's cardinality over the processes' parameters; an open-typed parameter is flagged, never counted as raw cardinality. Both read off the `.hd` before code exists. |
