...
| **Suppression** | `# honest: ignore\|disable\|enable RULE[: reason]` directives. A suppressed diagnostic is downgraded to `info` (visible, never dropped), so suppressions can't silently accumulate. `ignore` and `disable` must carry a reason after the colon (HC-SUP002); `enable` only closes a block, so it needs none. |
| **Adoption level** | Which rules the codebase *being checked* currently holds, declared as `[check] adoption` — Structural, Boundary, Typed, or Declared, each cumulative over the ones below it. Absent means Declared, the strictest, so silence never buys leniency. Findings above the level are downgraded to `info`, never dropped. A level is a named, closed set, unlike a per-rule dial, so "clean at level Boundary" means the same thing in every codebase that says it. Distinct from **conformance level**, which grades the checker, not the codebase. |
| **Dead directive** | A suppression whose rule never fires. It is indistinguishable from a live one, so it silently covers whatever violation the file grows next — HC-SUP001 reports it. Neither HC-SUP001 nor HC-SUP002 can itself be suppressed. |
| **Elementary process** | The unit honest-estimate counts: the smallest activity meaningful to a user OR enforceable as a system-boundary contract. Read from the `.hd`, with the boundary role as the authority (every `boundary_in`/`boundary_out`/`orchestrator` is one); where a module declares no role, an invoke-graph fallback treats a pure function no sibling invokes as a contract, and the module is flagged under-declared. The spec-tier grain, distinct from the per-`def` code-honesty census. |
| **Size readings (CFP / VFP / bits)** | honest-estimate's three deductive size numbers, never fused, all read off the `.hd` before code exists. CFP = COSMIC data movements (Entry/eXit/Read/Write) summed over the processes, floored at two each (Honest mapping, uncertified). VFP = the count of elementary processes, split IFPUG-countable vs beyond-IFPUG. Bits = specified case-distinction, log-2 of each closed set/vocabulary cardinality over the processes' input parameters, recursing into composed types; an open-typed parameter is flagged, never counted as raw cardinality. |
| **Estimate dimensions** | honest-estimate's full §13 artifact from `estimate()`: the three SIZE readings, plus projected COST (dollars, dated) and DURATION (wall-clock) bands, a build-time QUALITY proxy (mutation + escaped-defect density per process), and a JONES comparison (measured ratio to benchmark). Cost/duration/Jones take an injected priced/benchmark model; with no model they carry an explicit `uncalibrated — verify` marker — the estimator fabricates no constant. Size is deductive and certain; the rest are projections or proxies, never given size's certainty. |
