...
| honest-test | **Complete conformance level.** Generation engine, honesty checks, state-machine and mutation adequacy, the §11 orchestrating runner (`runner.py` discovery + `run_suite` + coverage.json) with its CLI (`cli.py`, `python -m honest_test.cli [src_dir]`), §7 component-isolation tests (`component_isolation.py`), and §6 honest-persist contract tests against a real in-memory database (`persist_contract.py`) all complete, plus the BDD runner. Deliberate §9.2 fault-path generation for chain coverage is a filed follow-up (a coverage-metric enhancement, not a conformance requirement). |
