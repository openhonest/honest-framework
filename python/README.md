...
| honest-observe | Pure core substantially complete; the CLI (`tail`/`inspect`/`query` with all filters, `read_event_log`, and the reusable `_entry`) and the config loader (`load_config`/`read_config` plus the dev-mode toggle resolvers, §11 / §9.5-9.6) are built and gated. The zero-config standalone bootstrap (adopter driver connection + registry from application code) and the default threshold-projection records are unbuilt. |
| honest-test | **Complete conformance level.** Generation engine, honesty checks, state-machine and mutation adequacy, the §11 orchestrating runner (`runner.py` discovery + `run_suite` + coverage.json) with its CLI (`cli.py`, `python -m honest_test.cli [src_dir]`), §7 component-isolation tests (`component_isolation.py`), §6 honest-persist contract tests against a real in-memory database (`persist_contract.py`), and §9.2 deliberate fault-path generation wired into `coverage.json` (`fault_paths.py`: read-the-guard, perturb, break-the-seam, with state-dependent exits disclosed) all complete, plus the BDD runner. honest-test is done. |
