...
| honest-observe | Pure core substantially complete; the CLI (`tail`/`inspect`/`query` with all filters, `read_event_log`, and the reusable `_entry`) is built and gated. The zero-config standalone bootstrap (adopter driver connection + registry from application code) and the default threshold-projection records are unbuilt. |
