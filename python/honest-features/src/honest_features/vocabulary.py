...
The vocabulary is static code: each flag declares its complete set of states and the default it holds
at startup. The flag state is ephemeral data, built from the vocabulary and threaded as a value — never
a module global — so `feature_state` stays a pure lookup and honest-features passes its own gate.
"""
...
def validate_vocabulary(features):
    ...
    and a `default` that is one of them. Returns ok(features) or a client fault naming the offenders. Pure."""
    bad = [flag for flag, spec in features.items() if len(spec["states"]) < 2 or spec["default"] not in spec["states"]]
    if bad:
...
def initial_state(features):
    """The flag state at startup: each flag at its declared default (section 3). Pure, no I/O. The
