...
import { parseVerbose, parseColon, parseClass, parseMagic, parseJson } from "./notation.js";

export function readConfig(element, vocabulary) {
  const prefix = resolvePrefix(element, vocabulary);
  if (!prefix) return null;
  // Merge the notations in precedence order: verbose is the base; the positional colon/class and the
  // unordered Type-Magic notations override it; JSON options are the most explicit and win last.
  const config = {
    ...parseVerbose(element, prefix),
    ...parseColon(element, prefix, vocabulary),
    ...parseClass(element, prefix, vocabulary),
    ...parseMagic(element, prefix, vocabulary),
