...
- [x] **The three deferred borderlines are now settled by inspection, not punted to the engine:**
  - `reservation_layer` (honest-type) — **was genuinely self-referential**: it drew its words from the module's own `_LAYER*` frozensets, so an emptied word still mapped correctly (the empty string sorts first and is itself a member) and the law passed. Fixed with a hand-written literal oracle pinning all three layers plus every word→layer mapping; the mutation engine confirms it load-bearing (92 caught). Committed `11bddae`.   - `checked_select`-vs-`select` (honest-persist `_probe_checked`:558) — **benign**: the comparison `checked_select(...)["ok"] == select(...)` leans on `select()` being independently pinned by suite.json hand-written `expect_sql`/`expect_params` (`q-select-star`/`-cols-where`/`-full`), and `checked_select` has its own `cq-select-ok`. Not circular.   - `manifest`-embeds-`schema` (honest-observe `_probe_event_log`:348) — **benign**: the embedded `table` is pinned against hand-written `expected_columns`/`expected_indexes`/nullability literals in the same probe (laws_ho.py 313–345); the embed check leans on that independent grounding.
