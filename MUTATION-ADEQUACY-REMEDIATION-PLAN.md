...
- [x] **A1 — Mutation operators (7) — now complete against §9.6.** (1) number shift covers float literals and hex/octal/binary integers via `int(text, 0)`, complex skipped — also fixed a latent crash on non-decimal literals (`bcd96af`); (2) condition flip negates a condition (`c`→`not (c)`) at if/elif/while/ternary/assert and every comprehension filter and match-guard — the `x`→`not x` case (`bcd96af`); (3) dict-key→sibling-key swap for keys of any kind, skipping a no-op duplicate (`4a79fba`); (4) line removal replaces a block's sole statement with `pass`, and branch-arm removal drops a droppable clause of every compound statement — if elif/else, for/while else, try except/else/finally (with the parseability guards), match case (`bcd96af`, `4a79fba`). Plus the docstring/annotation skips (`81cf45c`, `19453f5`). **Two deliberate boundaries remain, verified by probe:** a complex literal (`1j`) has no single 'one' to add and is not shifted; a module's *sole* top-level statement is left to deletion rather than `pass`-replaced (a module carries many top-level statements). Neither is a §9.6 operator gap.
